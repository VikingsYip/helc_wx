...
Page({
  ...
  clickToSave(e) {
    ...
    if (!(!validate.isEmpty(name)&&(validate.mobilePhone(code)||validate.telephone(code)))){
      show('名称不能为空和联系方式格式错误')
      return
    }
    ...
    if (this.data.currentIndex == -1) {
      POSTJSON('dropdown/add', { "code": code, "name": name, "pcode": "ADMIN_PHONE" }, true, (flag, data, des) => { 
        if (flag) {
          that.alertSuccess()
      ...
      POSTJSON('dropdown/edit', { "code": code, "name": name, "id": id }, true, (flag, data, des) => { 
        if (flag) {
          that.alertSuccess()
          that.requestToOrganList()
          wx.hideLoading()
